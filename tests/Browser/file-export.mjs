    ...
    blocks: [
        {
            ...
            rows: [
                {
                    id: 'row',
                    cells: { sku: '%product.sku%', name: '%product.name%' },
                },
            ],
        },
    ],
...
import { renderPreviewRows } from '@/components/sales/billing-file-context';
...
function App() {
    ...
    window.renderBillingPreview = renderPreviewRows;
    ...
    plugins: [
        {
            ...
            setup(builder) {
                ...
                builder.onLoad({ filter: /.*/, namespace: 'fixture' }, () => ({
                    contents: `export const usePage = () => ({props:{auth:{user:{id:7}},csrf_token:'test',locale:'fr'}});`,
                }));
            },
        },
    ],
...
const server = createServer(async (request, response) => {
    ...
    if (url.pathname === '/file-export-templates') {
        ...
        response.end(
            JSON.stringify({
                filename: `${body.template.filename}.${body.format}`,
                total: 2,
                sample_count: 2,
                line_count: 2,
                image_count: 0,
                limit: 1000,
                too_large: false,
                values: { 'product.sku': '0001', 'product.name': 'Plant' },
                rows: [
                    { heading: false, cells: [{ value: '0001', image: null }] },
                ],
            }),
        );
    else if (body?.check)
...
try {
    ...
    await page.evaluateOnNewDocument((legacy) => {
        localStorage.setItem(
            'product-export-templates:v1:7',
            JSON.stringify([
                { id: 'legacy-one', format: 'csv', template: legacy },
            ]),
        );
    }, template);
    ...
    await page.waitForFunction(
        () =>
            !document.querySelector(
                'select[aria-label="Configuration enregistrée"]',
            )?.disabled,
    );
    ...
    assert.equal(
        calls.filter((call) => call.body?.preview).length,
        1,
        'editing does not refresh',
    );
    assert.ok(
        await page.$('section[aria-label="Aperçu du fichier"] table'),
        'old preview remains',
    );
    assert.ok(
        await page.evaluate(() =>
            document.body.textContent.includes('aperçu à actualiser'),
        ),
    );
    ...
    assert.equal(
        calls.filter((call) => call.body?.preview).length,
        1,
        'export never refreshes preview',
    );
    assert.equal(
        calls.findLast((call) => call.body?.check).body.format,
        'xlsx',
        'export validates current format',
    );
    ...
    assert.equal(billing.shared, true);
    assert.equal(billing.enabled, true);
    assert.equal(billing.id, 'billing-custom');
    assert.deepEqual(
        billing.blocks,
        models.find((model) => model.id === originalId).template.blocks,
    );
    assert.equal(
        await page.$('section[aria-label="Aperçu du fichier"] table'),
        null,
    ...
    await page.evaluate(() =>
        [...document.querySelectorAll('label')]
            .find((label) => label.textContent.trim() === 'TSV')
            .click(),
    );
    await settle();
    assert.ok(
    ...
    const unknown = await page.evaluate((definition) => {
        definition.blocks[0].rows[0].cells.sku =
            '%calc:quantity*unit_price|decimal:2%';
        return window.compatibility(definition, () => ['%product.name%']);
    }, template);
    assert.deepEqual(unknown, ['%quantity%', '%unit_price%']);
    const imagePreview = await page.evaluate((definition) => {
        definition.extension = 'xlsx';
        definition.event = 'invoice';
        definition.blocks[0].columns = [{ id: 'image', name: 'Image' }];
        definition.blocks[0].rows = [
            { id: 'image-row', cells: { image: '%product.image%' } },
        ];
        return window.renderBillingPreview(definition);
    }, template);
    assert.equal(imagePreview[1].cells[0].image, '/images/placeholder.png');
    assert.deepEqual(errors, []);
    console.log(
        'PASS: migration, shared library, duplication, manual previews, current export, billing settings, compatibility.',
    );
} finally {
    await browser?.close();
