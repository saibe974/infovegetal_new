...
it('serializes manifest pipeline result to deterministic array', function (): void {
    $serializerClass = 'App\\Domain\\Sales\\Services\\OrderSalesShadowReleaseManifestPipelineResultSerializer';
    $serializer = new $serializerClass();

    $result = makeManifestPipelineResult();
    $array = $serializer->toArray($result);

    expect($array['readiness']['readiness_report']['status'])->toBe('ready')
        ->and($array['manifest']['schema_version'])->toBe('1.0')
        ->and($array['manifest']['release_action'])->toBe('approve_release')
        ->and($array['manifest_verification']['is_valid'])->toBeTrue()
        ->and($array['manifest_verification']['expected_manifest_id'])->toBe($array['manifest_verification']['actual_manifest_id']);
});

it('serializes manifest pipeline result to valid json', function (): void {
    $serializerClass = 'App\\Domain\\Sales\\Services\\OrderSalesShadowReleaseManifestPipelineResultSerializer';
    $serializer = new $serializerClass();
    ...
    expect($decoded['manifest']['status'])->toBe('ready')
        ->and($decoded['readiness']['execution_package_verification']['is_valid'])->toBeTrue()
        ->and($decoded['manifest_verification']['errors'])->toBe([]);
});

function makeManifestPipelineResult()
{
    ...
    return $runner->run(new $pipelineInputClass(
                ...
                governanceInput: new $governanceInputClass(
                    gateInput: new OrderSalesShadowGateInput(
                        batchInput: new OrderSalesShadowBatchInput(
                            orders: [
                                buildManifestPipelineSerializerOrderInput(1),
                                buildManifestPipelineSerializerOrderInput(2),
                            ],
                            generatedAtUtc: '2026-07-13T06:20:00Z',
                    ...
                    planGeneratedAtUtc: '2026-07-13T06:25:00Z',
                    limitedStartPercent: 10,
                    limitedEndPercent: 50,
                    hoursPerStep: 12,
                ),
                packageGeneratedAtUtc: '2026-07-13T06:30:00Z',
                envelopeGeneratedAtUtc: '2026-07-13T06:35:00Z',
                checksumAlgorithm: 'sha256',
            ),
            executionPlanGeneratedAtUtc: '2026-07-13T06:40:00Z',
            executionPackageGeneratedAtUtc: '2026-07-13T06:45:00Z',
            readinessGeneratedAtUtc: '2026-07-13T06:50:00Z',
        ),
        manifestGeneratedAtUtc: '2026-07-13T06:55:00Z',
        schemaVersion: '1.0',
    ));
}

function buildManifestPipelineSerializerOrderInput(int $lineId): OrderSalesCalculationInput
{
    ...
    return new OrderSalesCalculationInput(
        lineInputs: [
            new LineCalculationInput(
                ...
                priceReference: new ProductPriceReference(
                    productId: 1900 + $lineId,
                    dbProductId: 2000 + $lineId,
        ...
        inputContext: ['batch' => 'shadow-release-manifest-pipeline-serializer-test'],
        generatedAtUtc: '2026-07-13T06:20:00Z',
    );
}
