...
it('serializes manifest package governance result to deterministic array', function (): void {
    $serializerClass = 'App\\Domain\\Sales\\Services\\OrderSalesShadowReleaseManifestPackageGovernanceResultSerializer';
    $serializer = new $serializerClass();

    $result = makeManifestPackageGovernanceResult();
    $array = $serializer->toArray($result);

    expect($array['package_result']['package']['checksum_algorithm'])->toBe('sha256')
        ->and($array['package_result']['verification']['is_valid'])->toBeTrue()
        ->and($array['publication_decision']['action'])->toBe('publish_manifest')
        ->and($array['publication_decision']['approved'])->toBeTrue();
});

it('serializes manifest package governance result to valid json', function (): void {
    $serializerClass = 'App\\Domain\\Sales\\Services\\OrderSalesShadowReleaseManifestPackageGovernanceResultSerializer';
    $serializer = new $serializerClass();
    ...
    expect($decoded['publication_decision']['release_action'])->toBe('approve_release')
        ->and($decoded['package_result']['package']['manifest_pipeline']['manifest']['schema_version'])->toBe('1.0')
        ->and($decoded['package_result']['package']['manifest_pipeline']['manifest_verification']['is_valid'])->toBeTrue();
});

function makeManifestPackageGovernanceResult()
{
    $runnerClass = 'App\\Domain\\Sales\\Services\\OrderSalesShadowReleaseManifestPackageGovernanceRunner';
                    ...
                    governanceInput: new $governanceInputClass(
                        gateInput: new OrderSalesShadowGateInput(
                            batchInput: new OrderSalesShadowBatchInput(
                                orders: [
                                    buildManifestPackageGovernanceSerializerOrderInput(1),
                                    buildManifestPackageGovernanceSerializerOrderInput(2),
                                ],
                                generatedAtUtc: '2026-07-13T14:20:00Z',
                        ...
                        planGeneratedAtUtc: '2026-07-13T14:25:00Z',
                        limitedStartPercent: 10,
                        limitedEndPercent: 50,
                        hoursPerStep: 12,
                    ),
                    packageGeneratedAtUtc: '2026-07-13T14:30:00Z',
                    envelopeGeneratedAtUtc: '2026-07-13T14:35:00Z',
                    checksumAlgorithm: 'sha256',
                ),
                executionPlanGeneratedAtUtc: '2026-07-13T14:40:00Z',
                executionPackageGeneratedAtUtc: '2026-07-13T14:45:00Z',
                readinessGeneratedAtUtc: '2026-07-13T14:50:00Z',
            ),
            manifestGeneratedAtUtc: '2026-07-13T14:55:00Z',
            schemaVersion: '1.0',
        ),
        packageGeneratedAtUtc: '2026-07-13T15:00:00Z',
        checksumAlgorithm: 'sha256',
    ));
}

function buildManifestPackageGovernanceSerializerOrderInput(int $lineId): OrderSalesCalculationInput
{
    ...
    return new OrderSalesCalculationInput(
        lineInputs: [
            new LineCalculationInput(
                ...
                priceReference: new ProductPriceReference(
                    productId: 3300 + $lineId,
                    dbProductId: 3400 + $lineId,
        ...
        inputContext: ['batch' => 'shadow-release-manifest-package-governance-result-serializer-test'],
        generatedAtUtc: '2026-07-13T14:20:00Z',
    );
}
