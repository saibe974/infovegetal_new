...
it('runs manifest package governance and returns publication decision', function (): void {
                    ...
                    governanceInput: new $governanceInputClass(
                        gateInput: new OrderSalesShadowGateInput(
                            batchInput: new OrderSalesShadowBatchInput(
                                orders: [
                                    buildManifestPackageGovernanceRunnerOrderInput(1),
                                    buildManifestPackageGovernanceRunnerOrderInput(2),
                                ],
                                generatedAtUtc: '2026-07-13T13:20:00Z',
                        ...
                        planGeneratedAtUtc: '2026-07-13T13:25:00Z',
                        limitedStartPercent: 10,
                        limitedEndPercent: 50,
                        hoursPerStep: 12,
                    ),
                    packageGeneratedAtUtc: '2026-07-13T13:30:00Z',
                    envelopeGeneratedAtUtc: '2026-07-13T13:35:00Z',
                    checksumAlgorithm: 'sha256',
                ),
                executionPlanGeneratedAtUtc: '2026-07-13T13:40:00Z',
                executionPackageGeneratedAtUtc: '2026-07-13T13:45:00Z',
                readinessGeneratedAtUtc: '2026-07-13T13:50:00Z',
            ),
            manifestGeneratedAtUtc: '2026-07-13T13:55:00Z',
            schemaVersion: '1.0',
        ),
        packageGeneratedAtUtc: '2026-07-13T14:00:00Z',
        checksumAlgorithm: 'sha256',
    ));

    expect($result->packageResult->verification->isValid)->toBeTrue()
        ->and($result->publicationDecision->approved)->toBeTrue()
        ->and($result->publicationDecision->action)->toBe('publish_manifest')
        ->and($result->publicationDecision->releaseAction)->toBe('approve_release');
});

function buildManifestPackageGovernanceRunnerOrderInput(int $lineId): OrderSalesCalculationInput
{
    ...
    return new OrderSalesCalculationInput(
        lineInputs: [
            new LineCalculationInput(
                ...
                priceReference: new ProductPriceReference(
                    productId: 2900 + $lineId,
                    dbProductId: 3000 + $lineId,
        ...
        inputContext: ['batch' => 'shadow-release-manifest-package-governance-runner-test'],
        generatedAtUtc: '2026-07-13T13:20:00Z',
    );
}
