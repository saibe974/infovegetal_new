...
it('runs manifest package flow and returns valid verification', function (): void {
                    ...
                    governanceInput: new $governanceInputClass(
                        gateInput: new OrderSalesShadowGateInput(
                            batchInput: new OrderSalesShadowBatchInput(
                                orders: [
                                    buildManifestPackageRunnerOrderInput(1),
                                    buildManifestPackageRunnerOrderInput(2),
                                ],
                                generatedAtUtc: '2026-07-13T10:40:00Z',
                        ...
                        planGeneratedAtUtc: '2026-07-13T10:45:00Z',
                        limitedStartPercent: 10,
                        limitedEndPercent: 50,
                        hoursPerStep: 12,
                    ),
                    packageGeneratedAtUtc: '2026-07-13T10:50:00Z',
                    envelopeGeneratedAtUtc: '2026-07-13T10:55:00Z',
                    checksumAlgorithm: 'sha256',
                ),
                executionPlanGeneratedAtUtc: '2026-07-13T11:00:00Z',
                executionPackageGeneratedAtUtc: '2026-07-13T11:05:00Z',
                readinessGeneratedAtUtc: '2026-07-13T11:10:00Z',
            ),
            manifestGeneratedAtUtc: '2026-07-13T11:15:00Z',
            schemaVersion: '1.0',
        ),
        packageGeneratedAtUtc: '2026-07-13T11:20:00Z',
        checksumAlgorithm: 'sha256',
    ));

    expect($result->package->checksumAlgorithm)->toBe('sha256')
        ->and($result->package->manifestPipelineResult->manifestVerification->isValid)->toBeTrue()
        ->and($result->verification->isValid)->toBeTrue()
        ->and($result->verification->expectedChecksum)->toBe($result->verification->actualChecksum);
});

function buildManifestPackageRunnerOrderInput(int $lineId): OrderSalesCalculationInput
{
    ...
    return new OrderSalesCalculationInput(
        lineInputs: [
            new LineCalculationInput(
                ...
                priceReference: new ProductPriceReference(
                    productId: 2500 + $lineId,
                    dbProductId: 2600 + $lineId,
        ...
        inputContext: ['batch' => 'shadow-release-manifest-package-runner-test'],
        generatedAtUtc: '2026-07-13T10:40:00Z',
    );
}
