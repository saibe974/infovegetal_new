...
final class PromotionAudienceService
{
    ...
    public function eligibleQuery(User $user): Builder
    {
        ...
        $query = User::query()
            ->where('active', true)
            ->whereHas('roles', fn (Builder $roleQuery) => $roleQuery->where('name', 'client'));
    ...
    public function assertEligibleIds(User $user, array $ids): void
    {
        $ids = collect($ids)->map(fn ($id) => (int) $id)->unique()->values();
        $eligibleCount = $this->eligibleQuery($user)->whereKey($ids)->count();

        abort_unless($eligibleCount === $ids->count(), 403, 'Certains clients ne font pas partie de votre périmètre.');
    }
}
